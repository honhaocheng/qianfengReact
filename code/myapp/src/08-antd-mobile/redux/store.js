...
/*
流程是按照将来会生成一个叫kerwin这样的一个key值，按照存储到本地存储中，把普通的reducer持久化成persistedReducer，
然后再把persistedReducer生成store，再把普通store生成一个persistor，最后把普通store和持久化的store（persistor）都都导出去，就完事了
*/
const persistConfig = {
  key: 'kerwin',
  storage,
  // 设置白名单，只持久化CityReducer
...
// 这行代码的意思是说，我们要把我们的reducer按照我们这个配置（persistConfig）进行持久化reducer
...
const store  = createStore(persistedReducer, composeEnhancers(applyMiddleware(thunk, reduxPromise)))
// 生成持久化的store
